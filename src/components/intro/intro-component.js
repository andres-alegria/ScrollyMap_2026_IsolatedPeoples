...
const Intro = ({ title, subtitle, date, social, height }) => {
  ...
  const separatorClasses = 'h-px bg-back block flex-1 opacity-25';
  return (
    <div className="intro step absolute h-screen w-full bg-cover bg-no-repeat" style={heightStyle}>
      <div
        ...
        style={heightStyle}
      >
        <div className="container max-w-xl text-back text-center mb-10">
          <h1 className="title font-lora mb-10">{t(title)}</h1>
          <h2 className="text-lg font-regular">{t(subtitle)}</h2>
        </div>
        <div className="container max-w-xl text-back">
          {date && (
            <div className="text-xs font-bold text-back flex items-center mb-6">
        ...
        <div className="container max-w-sm text-black-100 text-center flex flex-col justify-center items-center">
